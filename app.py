...
def load_training_dataset():
    ...
    try:
        ...
        print(f"Files in current directory: {[f for f in os.listdir(current_dir) if f.endswith('.json')]}")
...
def find_json_response(user_input: str) -> dict:
    ...
    for item in TRAINING_DATA:
        ...
        important_words = {'insurance', 'policy', 'coverage', 'premium', 'funeral', 'burial', 'agent', 'quote', 'diabetes', 'COPD', 'exam', 'cost', 'price'}
        common_important = user_words.intersection(item_words).intersection(important_words)
...
def handle_stage_original(lead, user_msg: str) -> str:
    ...
    # 1. Greeting
    if stage == "greeting":
        if any(word in user_msg.lower() for word in ["yes", "y", "sure", "ok", "yeah", "yep"]):
            lead.stage = "ask_name"
            db.commit()
            return QUALIFICATION_QUESTIONS["ask_name"]
        elif any(word in user_msg.lower() for word in ["no", "n", "not interested", "nah"]):
...
def json_statistics():
    ...
    return jsonify({
        ...
        "unique_intents": len(set(item['intent'] for item in TRAINING_DATA)) if TRAINING_DATA else 0,
        "sample_intents": list(set(item['intent'] for item in TRAINING_DATA))[:10] if TRAINING_DATA else [],
        "responses_with_appointment_trigger": len([item for item in TRAINING_DATA if 'set_appointment' in item.get('trigger', [])]),
        "recruiting_responses": len([item for item in TRAINING_DATA if 'recruiting' in item.get('trigger', [])])
    })
